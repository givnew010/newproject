...
export default function Suppliers() {
  ...
  const createMutation = useMutation((s: any) => import('./lib/api').then(m => m.suppliersApi.create(s)));
  ...
  const handleSave = async () => {
    if (!form.name || form.name.trim() === '') { showError('اسم المورد مطلوب'); return; }
    try {
      if (editing) {
        const res = await updateMutation.mutate({ id: editing.id, sup: { ...form } });
        if (res?.success) {
          showSuccess(res?.message || 'تم تحديث المورد');
          refetch();
          setIsModalOpen(false);
        } else {
          showError(res?.error || 'فشل في تحديث المورد');
        }
      } else {
        const res = await createMutation.mutate(form);
        if (res?.success) {
          showSuccess(res?.message || 'تم إضافة المورد');
          refetch();
          setIsModalOpen(false);
        } else {
          showError(res?.error || 'فشل في إضافة المورد');
        }
      }
    } catch (err) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (res?.success) {
        showSuccess(res?.message || 'تم تعطيل المورد');
        refetch();
      } else {
        showError(res?.error || 'فشل في تعطيل المورد');
      }
    } catch (err) {
      showError('حدث خطأ أثناء العملية');
    }
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
            ...
            <table className="w-full text-right">
              <thead>
                <tr className="bg-surface-container-low/80 border-b border-surface-container-high">
                  <th className="px-5 py-3.5 text-[11px] font-bold text-on-surface-variant uppercase">المورد</th>
                  <th className="px-5 py-3.5 text-[11px] font-bold text-on-surface-variant uppercase hidden sm:table-cell">الهاتف</th>
                  <th className="px-5 py-3.5 text-[11px] font-bold text-on-surface-variant uppercase">المستحق</th>
                  ...
                  <tr>
                    <td colSpan={6} className="py-16 text-center">
                      <div className="flex flex-col items-center gap-3 text-on-surface-variant">
                        ...
                        <p className="text-sm font-medium">لا يوجد موردون مطابقون</p>
                      </div>
                    </td>
                  </tr>
                ) : (
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.98, y: 12 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.98 }} transition={{ type: 'spring', damping: 25 }} className="relative z-[70] w-full max-w-lg bg-white rounded-3xl shadow-2xl overflow-hidden">
              <div className="p-4 border-b border-surface-container-low flex items-center justify-between">
                <h3 className="font-extrabold">{editing ? 'تعديل المورد' : 'إضافة مورد'}</h3>
        ...
        {statementOpen && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.98, y: 12 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.98 }} transition={{ type: 'spring', damping: 25 }} className="relative z-[70] w-full max-w-2xl bg-white rounded-3xl shadow-2xl overflow-hidden">
              <div className="p-4 border-b border-surface-container-low flex items-center justify-between">
                <h3 className="font-extrabold">كشف حساب: {statementSupplier?.name}</h3>
      ...
      <ConfirmDialog
        ...
        title="تعطيل المورد"
        message="هل متأكد أنك تريد تعطيل هذا المورد؟"
        confirmLabel="تعطيل"
        cancelLabel="إلغاء"
