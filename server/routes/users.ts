...
const createUserSchema = z.object({
  username: z
    ...
    .regex(/^[a-zA-Z0-9_]+$/, "اسم المستخدم يجب أن يحتوي على أحرف إنجليزية وأرقام وشرطة سفلية فقط"),
  password: z
    .string()
...
const updateUserSchema = z.object({
  full_name: z.string().min(2, "الاسم الكامل حرفان على الأقل").max(100, "الاسم الكامل طويل جداً").optional(),
  role: roleEnum.optional(),
  is_active: z.number().int().min(0).max(1).optional(),
});

const changePasswordSchema = z.object({
  new_password: z.string().min(6, "كلمة المرور 6 أحرف على الأقل").max(100, "كلمة المرور طويلة جداً"),
...
router.get("/", (req: Request, res: Response): void => {
  ...
  const users = db
    .prepare(`SELECT id, username, full_name, role, is_active, created_at, last_login FROM users ${whereClause} ORDER BY created_at DESC`)
...
router.get("/:id", (req: Request, res: Response): void => {
  ...
  const user = db
    .prepare("SELECT id, username, full_name, role, is_active, created_at, last_login FROM users WHERE id = ?")
...
router.post("/", async (req: Request, res: Response): Promise<void> => {
  ...
  if (existing) {
    res.status(409).json({ success: false, error: `اسم المستخدم "${username}" مستخدم بالفعل` });
    return;
  }
  ...
  res.status(201).json({ success: true, data: { user: newUser }, message: `تم إنشاء المستخدم "${username}" بنجاح` });
});
...
router.put("/:id", (req: Request, res: Response): void => {
  ...
  const existingUser = db.prepare("SELECT * FROM users WHERE id = ?").get(id) as DbUser | undefined;
  if (!existingUser) {
    res.status(404).json({ success: false, error: "المستخدم غير موجود" });
    return;
  }
  ...
  if (parsed.is_active === 0 || parsed.role !== "admin") {
    if (existingUser.role === "admin") {
      ...
      if (wouldRemoveAdmin && activeAdminCount <= 1) {
        res.status(422).json({ success: false, error: "لا يمكن إجراء هذا التعديل — يجب وجود مدير نشط واحد على الأقل في النظام" });
        return;
      }
    }
  }
  ...
  res.status(200).json({ success: true, data: { user: updatedUser }, message: "تم تحديث بيانات المستخدم بنجاح" });
});
...
router.put("/:id/password", async (req: Request, res: Response): Promise<void> => {
  ...
  const user = db.prepare("SELECT id, username FROM users WHERE id = ?").get(id) as Pick<DbUser, "id" | "username"> | undefined;
  if (!user) {
    res.status(404).json({ success: false, error: "المستخدم غير موجود" });
    return;
  }
  ...
  res.status(200).json({ success: true, message: `تم تغيير كلمة مرور المستخدم "${user.username}" بنجاح` });
});
...
router.delete("/:id", (req: Request, res: Response): void => {
  ...
  if (id === req.user!.userId) {
    res.status(422).json({ success: false, error: "لا يمكنك تعطيل حسابك الخاص" });
    return;
  }
  ...
  if (user.role === "admin" && user.is_active === 1) {
    ...
    if (activeAdminCount <= 1) {
      res.status(422).json({ success: false, error: "لا يمكن تعطيل هذا الحساب — هو المدير النشط الوحيد في النظام" });
      return;
    }
  }
  ...
  res.status(200).json({ success: true, message: `تم تعطيل حساب المستخدم "${user.username}" بنجاح` });
});
